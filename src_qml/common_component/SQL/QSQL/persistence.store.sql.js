    ...
    function save(obj, tx, callback) {
        ...
        executeQueriesSeq(tx, additionalQueries, function () {
            ...
            if (obj._new) {
                ...
                obj._new = false;
                let results = tx.executeSql(sql, values);
                if (callback) callback(tx, results)
...
//                tx.executeSql(sql, values, callback, callback);
    ...
    function executeQueriesSeq(tx, queries, callback) {
        ...
        persistence.asyncForEach(queries, function (queryTuple, callback) {
            let results = tx.executeSql(queryTuple[0], queryTuple[1] ? queryTuple[1] : []);
            if (callback) {
                callback(tx, results)
            }
...
//            });
    ...
    persistence.DbQueryCollection.prototype.list = function (tx, callback) {
        ...
        session.flush(tx, function () {
            console.log('(persistence.store.sql)tip 708', sql)
            let the_ans = tx.executeSql(sql, args)
            let rows = []
            for (let i = 0; i < the_ans.rows.length; i++) {
                rows.push(the_ans.rows.item(i))
            }
...
//            });
    ...
    persistence.DbQueryCollection.prototype.destroyAll = function (tx, callback) {
        ...
        session.flush(tx, function () {
            let results = tx.executeSql(selectSql, args)
            ...
            let new_result = tx.executeSql(deleteSql, args2);
            if (callback) callback(tx, new_result)
...
//            }, callback);
    ...
    persistence.DbQueryCollection.prototype.count = function (tx, callback) {
        ...
        session.flush(tx, function () {
            console.log('(persistence.store.sql)tip 891', sql)
            let results = tx.executeSql(sql, args)
            callback(parseInt(results.rows.item(0).cnt, 10));
